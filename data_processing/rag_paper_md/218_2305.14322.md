...
中文翻译:
大型语言模型（LLM）凭借其庞大的参数量与全面的数据利用能力，显著推动了自然语言处理（NLP）领域的发展。然而现有LLM缺乏专用记忆单元，限制了其显式存储和调用知识以执行多样化任务的能力。本文提出RET-LLM创新框架，通过为LLM配备通用读写记忆单元，使其能够根据任务需求从文本中提取、存储和调用知识。受戴维森语义理论启发，我们采用三元组形式进行知识提取与存储。该记忆单元具备可扩展性、可聚合性、可更新性和可解释性等特性。定性评估表明，在问答任务中本框架显著优于基线方法。此外，该框架在时序性问答任务中展现出强劲性能，证明了其有效管理时间依存信息的能力。
